...
RSpec.describe Spree::VerificationCode do
  ...
  describe '.usable_for' do
    it 'answers the code the number may still be proved with' do
      ...
      expect(described_class.usable_for(phone: phone, purpose: 'account')).to eq(code)
    end
    ...
    it 'answers nothing once the code is spent' do
      ...
      expect(described_class.usable_for(phone: phone, purpose: 'account')).to be_nil
    end
    ...
    it 'answers the newest code, not an older one still in its window' do
      ...
      expect(described_class.usable_for(phone: phone, purpose: 'account')).to eq(newest)
    end
  ...
  describe '.consume' do
    ...
    it 'spends the code, and refuses the second spend' do
      ...
      expect(described_class.consume(phone: phone, purpose: 'account', code: '123456')).to be_present
      expect(described_class.consume(phone: phone, purpose: 'account', code: '123456')).to be_nil
    end

    it 'refuses a code that does not match' do
      issue

      expect(described_class.consume(phone: phone, purpose: 'account', code: '999999')).to be_nil
      expect(described_class.usable_for(phone: phone, purpose: 'account')).to be_present
    end
    ...
    it 'refuses a code issued for the other family' do
      ...
      expect(described_class.consume(phone: phone, purpose: 'account', code: '123456')).to be_nil
      expect(described_class.consume(phone: phone, purpose: 'payment', code: '123456')).to be_present
    end

    it 'reads any spelling of the number as the same number' do
      issue

      expect(described_class.consume(phone: '+86 138-0013-8000', purpose: 'account', code: '123456')).to be_present
    end
  end
