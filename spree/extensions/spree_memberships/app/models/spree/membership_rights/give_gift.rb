module Spree
  module MembershipRights
    ...
    class GiveGift < Spree::MembershipRight
      ...
      validate :gift_settings_must_hold_together, if: -> { will_save_change_to_preferences? }
      validate :gift_promotions_must_belong_to_the_store, if: -> { will_save_change_to_preferences? }
      ...
      # The coupons this gift offers, in the order claims take them. A coupon
      # the operator has since deleted drops out rather than failing the read.
      #
      # @return [Array<Spree::Promotion>]
      def gift_promotions
        Array(preferred_gift_promotion_ids).filter_map { |id| Spree::Promotion.find_by_prefix_id(id) }
      end

      # @return [Boolean] whether a claim here hands a coupon over
      def coupon_gift?
        preferred_gift_mode.to_s == 'coupon'
      end

      # @return [Spree::Memberships::YearGift] this member's own state of the
      #   gift: what they may still claim, and which coupons are left
      ...
      def gift_settings_must_hold_together
        errors.add(:preferences, :invalid) if preferred_yearly_limit.to_i < 1
        return if MODES.include?(preferred_gift_mode.to_s)

        errors.add(:preferences, :invalid)
      end

      # The entry coupon's own check, over a list: a pool in another store's
      # promotion is a code this tier must not draw. Read only while the
      # preferences are being written, so a right whose promotion has since gone
