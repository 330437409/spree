module Spree
  ...
  # that engine said.
  # Loaded by the rake task that runs it and by its own spec; the rake file
  # itself keeps only the task, because a class defined there cannot be tested.
  class CrmMembershipImport
    ...
    def port_memberships
      ...
      crm_memberships.order(end_date: :desc, id: :desc).each do |row|
        ...
        if group_id.blank? || row.customer_id.blank? || row.store_id.blank?
          puts "  skipped #{row.id}: it has no tier to grant, or no store to grant it in."
          skipped += 1
          next
        end
        ...
        moved += 1
      # StatementInvalid as well as the two the ladder's own rules raise: the
      # legacy tables allow a null a NOT NULL column here does not, and one bad
      # row must not take the whole convergence down with it.
