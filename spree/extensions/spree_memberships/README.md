...
| `GET /api/v3/store/membership_surprise_packet?tier_id=` | The packet of coupons a tier hands over, asked about a package the customer has not bought yet — the settlement page reads it before the purchase, and the member centre reads the same payload off the right. Each coupon's worth is read from the promotion it draws on; its copies and its cadence are the tier's own. `null` for a tier that grants none |
