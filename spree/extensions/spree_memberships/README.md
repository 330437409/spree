...
| `Spree::Memberships::SurprisePacket`, `::SurpriseCoupon` | The tier's packet of coupons, read from the promotions it draws on and the counts its right carries |
...
| `GET /api/v3/store/membership_purchase_checks?tier_id=` | What buying that package means for the terms the customer already holds. Answers `checks`, empty in the ordinary case: `overlap` names the term the purchase waits behind and the instant the new one begins, `open_ended` says another tier held with no end will refuse the card, and `adds_no_time` says this tier is held with no end so the purchase would add nothing. Advisory — a purchase proceeds on any answer. Both sides are the store's own: another store's package is a 404, and a term held against another store's tier is not this store's business |
...
| `GET /api/v3/store/membership_surprise_packet?tier_id=` | The packet of coupons a tier hands over, asked about a package the customer has not bought yet — the settlement page reads it before the purchase, and the member centre reads the same payload off the right. Each coupon's worth is read from the promotion it draws on — its `discount_type` is null where the promotion states none this can read, so the card claims no figure rather than a nought — and its copies and its cadence are the tier's own. `null` for a tier that grants none |
| `GET /api/v3/store/customers/me/membership` | The customer's own rung, its sections and how many rights it carries. A customer in no tier is answered a null tier rather than refused. A right whose kind has something of its own to say carries it here — the annual gift's coupons and both of its counts ride the entry as `gift`, and the surprise packet's coupons as `packet` |
