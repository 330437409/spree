// Typelizer digest 1af16aab3e7d049b2f8d4df53cfeff48
//
// DO NOT MODIFY: This file was automatically generated by Typelizer.
import type {SpreeApiV3PromotionCandidate, Seller, OptionValue, DigitalLink, TaxLine, Variant, TaxCategory} from '@/types'

interface LineItem {
  id: string;
  variant_id: string;
  seller_id: string | null;
  preorder: boolean;
  preorder_ships_at: string | null;
  promotion_id: string | null;
  promotion_candidates: Array<SpreeApiV3PromotionCandidate>;
  quantity: number;
  currency: string;
  name: string;
  slug: string;
  options_text: string;
  selected: boolean;
  price: string;
  display_price: string;
  total: string;
  display_total: string;
  adjustment_total: string;
  display_adjustment_total: string;
  additional_tax_total: string;
  display_additional_tax_total: string;
  included_tax_total: string;
  display_included_tax_total: string;
  discount_total: string;
  display_discount_total: string;
  pre_tax_amount: string;
  display_pre_tax_amount: string;
  discounted_amount: string;
  display_discounted_amount: string;
  display_compare_at_amount: string | null;
  compare_at_amount: string | null;
  thumbnail_url: string | null;
  seller?: Seller;
  option_values: Array<OptionValue>;
  digital_links: Array<DigitalLink>;
  tax_lines?: Array<TaxLine>;
  metadata: Record<string, unknown>;
  price_source: string | null;
  created_at: string;
  updated_at: string;
  catalog_price: string | null;
  price_list_id: string | null;
  price_list_name: string | null;
  catalog_id: string | null;
  catalog_name: string | null;
  cost_price: string | null;
  tax_category_id: string | null;
  variant?: Variant;
  tax_category?: TaxCategory;
}

export default LineItem;
