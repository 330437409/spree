...
| `spree/extensions` | Ruby gems built in this fork, loaded as path gems like the providers — foundation gems first (`spree_administrative_divisions`: the Chinese administrative tree as imported reference data), then the feature gems the mini-program plans describe. Wired in three places or their specs never run: the starter's `server/Gemfile`, the CI project list, and the gem's own `Gemfile`. |
...
**A nested serializer's type resolves only in the packages that generate it.** A store-side serializer named in a `typelize` declaration (e.g. `transfer: 'MembershipCardTransfer | null'`) typechecks in `@spree/sdk` and breaks `@spree/admin-sdk`: an admin serializer extending the store one inherits the declaration, while the admin barrel has no such type — `error TS2305: Module '"@/types"' has no exported member`. **Restating it on the admin serializer is the same mistake**, not a fix (e.g. `areas: 'MembershipBannerArea[]'` on an admin banner): an extension gem's serializer is store-side only, so its type is missing from the admin barrel by construction, and the admin declaration fails on its own. Leave the admin side loose, and declare the **element** type rather than the array — `areas: 'Record<string, unknown>'`, never `'Array<Record<string, unknown>>'`: typelizer unions the parent's multi-attribute marks onto the subclass, so an `Array<…>` string is wrapped a second time and the generated type comes out one Array too deep. Run **both** SDK typechecks after regenerating (`cd packages/sdk && pnpm typecheck`, then `packages/admin-sdk`); CI's `pnpm turbo typecheck` is what catches it otherwise.
...
* the fork's own gems document themselves: a new model or endpoint in `spree/extensions/*` (or `spree/providers/*`) is documented in that gem's `README.md` and in its plan under `docs/plans/`, not by a page in `docs/developer/` — that tree is upstream's and is deployed publicly (ruled 2026-09-19, `fork-decisions.md`). A change to core or the API that upstream's readers would look for still gets a page there.
