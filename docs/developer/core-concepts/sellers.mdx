...
## Shopping at one seller's storefront

A customer browsing a single seller's storefront — a shop-inside-the-marketplace page, a local site, a brand page — sends `X-Spree-Seller-Id` on every Store API request. The seller is then the scope the request is answered in: catalogue reads, prices and delivery areas all resolve for it, the same way a channel scopes them.

```bash cURL
# The storefront that serves this seller sends this on every request
curl 'https://marketplace.example.com/api/v3/store/products' \
  -H 'X-Spree-API-Key: pk_xxx' \
  -H 'X-Spree-Seller-Id: sel_xxx'
```

<Note>
Two rules differ from the channel header beside it, and both are deliberate.

**Naming no seller is not an error.** The request is simply unscoped, and a read that needs a seller answers nothing rather than falling back to a default one — a price or a stock figure answered for the wrong seller is worse than an empty page.

**Naming a seller this store does not have is a 404** (`seller_not_found`), never a silent fall back to the marketplace's own catalogue.
</Note>

On the Ruby side the resolved seller is `Spree::Current.seller`, beside `Spree::Current.store` and `Spree::Current.channel`. Read it rather than resolving the seller again — [Channels](/developer/core-concepts/channels) covers how the sibling scope resolves.

<Note>
The Store SDK sends `X-Spree-Channel` from its `channel` config option but has no equivalent for the seller yet, so a storefront using the SDK calls the API directly for now.
</Note>
...
A refund never edits an earning. `refund.created` writes a second row of kind `refund_reversal` against the same order, so what a seller has earned is always the sum of their transfers, and a reversal that lands after a payout closed falls into the next period rather than rewriting a settlement that already happened. Every credit the order carries is reversed, each with its own row — the earning's and, when there is one, the [subsidy's](#the-subsidy-what-the-platform-funded).

#### The subsidy — what the platform funded

A marketplace can promise a price the seller never agreed to — a member or VIP price is the usual case. The customer pays less, and the seller is still paid as though the discount had not happened, because the platform funded it. That shortfall is a `Spree::SellerTransfer` of kind `subsidy`, written beside the earning: the reduction less the commission the platform did not charge on it, computed from the sale's own commission rows and stored rather than recomputed.

Whoever promised the funding says what it was worth. An extension contributes the reduction through the `funded_discounts` hook on `Spree::SellerTransfers::Create`, and core writes the row, hands it to the payout provider and reverses it on a refund — at exactly the fraction the earning comes back at, so a refunded order returns the same proportion of everything the platform paid. A discount the *seller* gave is contributed by nobody: a promotion, a store-wide price list or a price the seller set already comes out of their own earning.

```ruby
# An extension that funds part of a price says so here. The reduction is gross
# and per line; the metadata is what an operator reading the ledger needs to
# see why the seller was credited.
class MyApp::FundedDiscount
  def call(workflow)
    reductions = MyApp::Reductions.for(workflow.order)   # { line_item_id => amount }
    return {} if reductions.empty?

    { discounts: reductions, metadata: { 'funded_by' => 'my_promise' } }
  end
end

Spree.hooks.register('seller_transfers.create.funded_discounts', 'MyApp::FundedDiscount')
```
